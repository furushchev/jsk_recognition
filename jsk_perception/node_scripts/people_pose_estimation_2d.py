...
class PeoplePoseEstimation2D(ConnectionBasedTransport):
    ...
    def _load_chainer_model(self):
        ...
        #
        if self.gpu >= 0:
            with chainer.cuda.get_device_from_id(self.gpu):
                self.pose_net.to_gpu()
                if self.use_hand:
                    self.hand_net.to_gpu()
    ...
    def _pose_estimate_chainer_backend(self, bgr_img):
        if self.gpu >= 0:
            with chainer.cuda.get_device_from_id(self.gpu):
                return self._pose_estimate_chainer_backend_impl(bgr_img)
        else:
            return self._pose_estimate_chainer_backend_impl(bgr_img)

    def _pose_estimate_chainer_backend_impl(self, bgr_img):
        xp = self.pose_net.xp
    ...
    def _hand_estimate_chainer_backend(self, bgr, people_joint_positions):
        if self.gpu >= 0:
            with chainer.cuda.get_device_from_id(self.gpu):
                return self._hand_estimate_chainer_backend_impl(bgr, people_joint_positions)
        else:
            return self._hand_estimate_chainer_backend_impl(bgr, people_joint_positions)

    def _hand_estimate_chainer_backend_impl(self, bgr, people_joint_positions):
